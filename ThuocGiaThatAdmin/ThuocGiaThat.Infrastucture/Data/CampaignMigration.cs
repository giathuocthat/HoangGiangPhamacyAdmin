    ...
    public static class CamPaignMigration
    {
        public static async Task InitializeAsync(IServiceProvider serviceProvider, IConfiguration configuration)
        {
            ...
            try
            {
                ...
                candidatePaths.Add(Path.Combine(baseDir, "Scripts", "campaign.sql"));
                candidatePaths.Add(Path.Combine(baseDir, "campaign.sql"));

                string? sqlFile = candidatePaths.FirstOrDefault(File.Exists);

                if (sqlFile == null)
                {
                    logger.LogWarning("campaign.sql not found in any expected location. Checked: {paths}", string.Join(", ", candidatePaths));
                    return;
                }
