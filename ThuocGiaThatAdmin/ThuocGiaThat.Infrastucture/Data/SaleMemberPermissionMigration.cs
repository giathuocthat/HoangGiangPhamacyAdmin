...
{
    public class SaleMemberPermissionMigration
    {
        public static async Task InitializeAsync(IServiceProvider serviceProvider, IConfiguration configuration)
        {
            ...
            try
            {
                var saleMemberRole = SaleMemberPermissions.Role;
                var claimType = SaleMemberPermissions.ClaimType;
                var saleMemberPermission = SaleMemberPermissions.GetAllPermissions();

                // Ensure SaleManager role exists
                var role = await roleManager.FindByNameAsync(saleMemberRole);

                if (role == null)
                {
                    logger.LogWarning("SaleMember role not found. RoleClaims migration skipped. Please ensure UserMigration runs before RoleClaimsMigration.");
                    return;
                }
